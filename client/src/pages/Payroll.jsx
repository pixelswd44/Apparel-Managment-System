...
import {
  ...
  Phone, Mail, CreditCard, Briefcase, Building2, Calendar,
...
function EmployeePanel({ employee, onClose, onEdit, onRefresh, embedded = false }) {
  ...
  const [advances, setAdvances] = useState(employee.advances || []);
  ...
  async function confirmDeleteAdvance() {
    ...
    setAdvances(p => p.filter(a => a.id !== delAdvTarget));
      ...
      <div className={embedded ? 'flex flex-col h-full overflow-hidden' : 'w-full max-w-md bg-white h-full shadow-2xl flex flex-col animate-drawer'} onClick={embedded ? undefined : e => e.stopPropagation()}>
        ...
        <div className="px-6 py-5 border-b border-slate-100 flex items-start justify-between">
          <div>
            <div className="flex items-center gap-2">
              <div className={`w-2 h-2 rounded-full ${STATUS_DOT[employee.status] || 'bg-slate-400'}`} />
        ...
        {advModal && (
          <AdvanceModal
            ...
            onSave={saved => {
              if (advModal === 'new') setAdvances(p => [saved, ...p]);
              else setAdvances(p => p.map(a => a.id === saved.id ? saved : a));
              onRefresh();
            }}
          />
        )}
