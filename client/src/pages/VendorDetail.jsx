...
function PaymentHistory({ vendorId, data, onRefresh }) {
  ...
  // Flatten all payments — from process projects OR shipping projects
  const allPayments = isFreight
    ? (data.shippingProjects || [])
        .flatMap(sp => (sp.payments || []).map(p => ({ ...p, project_title: sp.project_title })))
        .sort((a, b) => new Date(b.paid_at) - new Date(a.paid_at))
    : (data.projects || [])
        .flatMap(pv => (pv.payments || []).map(p => ({ ...p, project_title: pv.project_title, project_id: pv.project_id })))
        .sort((a, b) => new Date(b.paid_at) - new Date(a.paid_at));
          ...
          {allPayments.map(p => (
            <div key={p.id} className="px-5 py-3">
              <div className="flex items-start justify-between gap-2">
                {/* Left: date + project + method */}
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="text-sm font-bold text-slate-900">{pkr(p.amount)}</span>
                  ...
                  <p className="text-xs text-slate-500 mt-0.5">{fmt(p.paid_at)}</p>
                  <p className="text-xs text-indigo-600 font-medium truncate">{p.project_title}</p>
                  {(p.reference || p.notes) && (
