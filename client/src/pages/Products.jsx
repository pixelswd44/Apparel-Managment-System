...
const DRAWER_TABS = ['Overview', 'Orders', 'Images', 'Calculate Price'];
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {tab === 'Overview' && (
            <div className="grid grid-cols-5 divide-x divide-slate-100 min-h-full">
              ...
              <div className="col-span-2 px-6 py-5 space-y-5 bg-slate-50/50">
                <div>
                    ...
                    if (prices.length === 0) {
                      return (
                        <p className="text-xs text-slate-400 italic">
                          No prices set.{' '}
                          {product.product_type !== 'service' && (
                            <button type="button" onClick={() => setTab('Calculate Price')}
                              className="text-indigo-500 hover:underline font-medium">
                              Use Calculate Price tab
                            </button>
                          )}
                        </p>
                      );
                ...
                {/* Cost Breakdown — hidden for services */}
                {product.product_type !== 'service' && <div>
                  <div className="flex items-center justify-between mb-3">
                    <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Cost Breakdown</p>
                    {calcTemplate && <span className="text-xs text-slate-400 italic truncate max-w-[100px]">{calcTemplate.name}</span>}
                  </div>
                  {!calcTemplate ? (
                    <p className="text-xs text-slate-400 italic">
                      No calculation saved yet.{' '}
                      <button onClick={() => setTab('Calculate Price')} className="text-indigo-500 hover:underline font-medium">
                        Use Calculate Price tab
                      </button>{' '}to add one.
                    </p>
                  ) : (() => {
                    let savedCosts = {};
                    try { savedCosts = JSON.parse(calcTemplate.costs || '{}'); } catch {}
                    const cur = calcTemplate.currency || 'PKR';
                    const cRow = calcCurrencies.find(c => c.code === cur);
                    const sym = cRow?.symbol || cur;
                    const cFmt = v => v > 0 ? `${sym}${v.toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 2 })}` : '—';
                    const fp  = parseFloat(savedCosts.__fabric_price) || 0;
                    const ppu = parseFloat(savedCosts.__pieces_per_unit) || 0;
                    const fu  = savedCosts.__fabric_unit || 'KG';
                    const fabricCost = fp > 0 && ppu > 0 ? fp / ppu : 0;
                    const rows = calcLabels.map(l => ({ label: l.label, key: l.key, val: parseFloat(savedCosts[l.key]) || 0 }));
                    const unitCostSaved = fabricCost + rows.reduce((s, r) => s + r.val, 0);
                    const profitPctSaved = parseFloat(calcTemplate.profit_margin) || 0;
                    const sellPriceSaved = unitCostSaved * (1 + profitPctSaved / 100);
                    return (
                      <div className="bg-white border border-slate-200 rounded-xl overflow-hidden text-xs">
                        {fabricCost > 0 && (
                          <div className="flex justify-between items-center px-3 py-2 border-b border-slate-100 bg-indigo-50/50">
                            <span className="text-indigo-600 font-medium">Fabric ({fu})</span>
                            <span className="font-semibold text-indigo-700">{cFmt(fabricCost)}</span>
                          </div>
                        )}
                        {rows.filter(r => r.val > 0).map((r, i, arr) => (
                          <div key={r.key} className={`flex justify-between items-center px-3 py-2 ${i < arr.length - 1 ? 'border-b border-slate-100' : ''}`}>
                            <span className="text-slate-500">{r.label}</span>
                            <span className="font-semibold text-slate-700">{cFmt(r.val)}</span>
                          </div>
                        ))}
                        <div className="flex justify-between items-center px-3 py-2.5 bg-slate-50 border-t border-slate-200">
                          <span className="font-bold text-slate-600 uppercase tracking-wider">Unit Cost</span>
                          <span className="font-bold text-slate-900">{cFmt(unitCostSaved)}</span>
                        </div>
                        {profitPctSaved > 0 && <>
                          <div className="flex justify-between items-center px-3 py-2 border-t border-slate-100">
                            <span className="text-slate-500">Margin ({profitPctSaved}%)</span>
                            <span className="font-semibold text-emerald-600">+{cFmt(sellPriceSaved - unitCostSaved)}</span>
                          </div>
                          <div className="flex justify-between items-center px-3 py-2.5 bg-indigo-50 border-t border-indigo-100">
                            <span className="font-bold text-indigo-700 uppercase tracking-wider">Sell Price</span>
                            <span className="font-bold text-indigo-700">{cFmt(sellPriceSaved)}</span>
                          </div>
                        </>}
                        <div className="px-3 py-1.5 border-t border-slate-100 bg-slate-50/50">
                          <span className="text-slate-400 text-2xs">Currency: <strong>{cur}</strong></span>
                        </div>
                      </div>
                    );
                  })()}
            ...
            <div className="px-5 py-4 space-y-4">
              ...
              {defaultCur ? (
                <div className="flex items-center gap-3 bg-indigo-50 border border-indigo-100 rounded-xl px-4 py-2.5">
                  <Calculator size={14} className="text-indigo-600 flex-shrink-0" />
                  <span className="text-xs text-indigo-700">
                    Calculating in <span className="font-bold">{defaultCur.code}{defaultCur.symbol ? ` (${defaultCur.symbol})` : ''}</span> — your default currency
                  </span>
                  {calcCurrencies.length > 1 && (
                    <span className="ml-auto text-xs text-indigo-500 flex-shrink-0">
                      +{calcCurrencies.length - 1} currencies auto-saved
                    </span>
                  )}
                </div>
              ) : (
                <div className="bg-amber-50 border border-amber-200 rounded-xl px-4 py-2.5 text-xs text-amber-700">
                  No default currency set — go to Settings → Currencies to set one.
                </div>
              )}

              {/* Saved calculations */}
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 space-y-2">
                <p className="text-2xs font-semibold text-slate-400 uppercase tracking-widest">Saved Calculations</p>
                <div className="flex gap-2">
                  <div className="relative flex-1">
                    <select value={selectedId} onChange={e => loadCalc(e.target.value)}
                      className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-indigo-400 bg-white cursor-pointer appearance-none">
                      <option value="">— New Calculation —</option>
                      {savedCalcs.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                    </select>
                    <ChevronDown size={12} className="absolute right-2.5 top-1/2 -translate-y-1/2 text-slate-400 pointer-events-none" />
                  </div>
                  {selectedId && (
                    delCalcId === parseInt(selectedId) ? (
                      <div className="flex items-center gap-1 bg-rose-50 border border-rose-200 rounded-lg px-2">
                        <button onClick={() => handleDeleteCalc(parseInt(selectedId))} className="text-xs text-rose-600 font-semibold px-1">Del</button>
                        <button onClick={() => setDelCalcId(null)} className="text-xs text-slate-400 px-1">No</button>
                      </div>
                    ) : (
                      <button onClick={() => setDelCalcId(parseInt(selectedId))}
                        className="p-2 text-slate-300 hover:text-rose-500 border border-slate-200 rounded-lg transition-all">
                        <Trash2 size={13} />
                      </button>
                    )
                  )}
                ...
                {!selectedId ? (
                  <div className="flex gap-2">
                    <input type="text" value={newCalcName} onChange={e => setNewCalcName(e.target.value)}
                      onKeyDown={e => e.key === 'Enter' && newCalcName.trim() && handleSaveCalc()}
                      placeholder="Name this calculation to save…"
                      className="flex-1 border border-slate-200 rounded-lg px-2.5 py-1.5 text-sm outline-none focus:border-indigo-400 bg-white placeholder:text-slate-400" />
                    <button onClick={handleSaveCalc} disabled={!newCalcName.trim() || savingCalc}
                      className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${saveSuccess ? 'bg-emerald-100 text-emerald-700' : 'bg-indigo-600 text-white hover:bg-indigo-700 disabled:opacity-50'}`}>
                      {savingCalc ? '…' : saveSuccess ? '✓' : <><Save size={11} />Save</>}
                    </button>
                  </div>
                ) : (
                  <div className="flex items-center justify-between">
                    <p className="text-xs text-slate-400">Editing: <span className="font-semibold text-slate-600">{savedCalcs.find(c => c.id === parseInt(selectedId))?.name}</span></p>
                    <button onClick={handleSaveCalc} disabled={savingCalc}
                      className={`flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${saveSuccess ? 'bg-emerald-100 text-emerald-700' : 'bg-indigo-600 text-white hover:bg-indigo-700 disabled:opacity-50'}`}>
                      {savingCalc ? '…' : saveSuccess ? '✓ Saved' : <><Save size={11} />Update</>}
                    </button>
                  </div>
                )}
              ...
              <div className="border border-slate-200 rounded-xl overflow-hidden">
                <div className="bg-indigo-50 px-4 py-2 border-b border-indigo-100 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-xs font-semibold text-indigo-700 uppercase tracking-wider">Fabric / Material</p>
                  </div>
                  <span className="text-2xs text-indigo-400">cost = price ÷ pieces per {fabricUnit}</span>
                </div>
                <div className="p-4 space-y-3">
                  <div className="grid grid-cols-3 gap-3">
                    <div>
                      <label className="block text-2xs font-semibold text-slate-400 uppercase tracking-widest mb-1.5">Unit</label>
                      <select value={fabricUnit} onChange={e => setFabricUnit(e.target.value)}
                        className="w-full border border-slate-200 rounded-xl px-3 py-2 text-sm outline-none focus:border-indigo-400 bg-white cursor-pointer">
                        {FABRIC_UNITS.map(u => <option key={u}>{u}</option>)}
                      </select>
                    </div>
                    <div>
                      <label className="block text-2xs font-semibold text-slate-400 uppercase tracking-widest mb-1.5">
                        {currSym} / {fabricUnit}
                      </label>
                      <div className="relative">
                        <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-xs font-semibold pointer-events-none">{currSym}</span>
                        <input type="number" min="0" step="any" value={fabricPrice}
                    ...
                    <div>
                      ...
                      <input type="number" min="0.001" step="any" value={piecesPerUnit}
                        onChange={e => setPiecesPerUnit(e.target.value)}
                        placeholder="e.g. 3.5"
                        className={inputCls} />
                    </div>
                  </div>
                  <div className={`flex items-center justify-between px-3 py-2.5 rounded-xl text-sm border ${fabricCostPerPiece > 0 ? 'bg-indigo-50 border-indigo-100' : 'bg-slate-50 border-slate-100'}`}>
                    <span className="text-xs text-slate-500">
                      {parse(fabricPrice) > 0 && parse(piecesPerUnit) > 0
                        ? `${fmtCalc(parse(fabricPrice))} ÷ ${piecesPerUnit} pcs`
                        : 'Fabric cost per piece (auto-calculated)'}
                    </span>
                    <span className={`font-bold text-sm ${fabricCostPerPiece > 0 ? 'text-indigo-700' : 'text-slate-300'}`}>
                      {fabricCostPerPiece > 0 ? fmtCalc(fabricCostPerPiece) : '—'}
                    </span>
                  </div>
                </div>
              </div>
              ...
              <div className="border border-slate-200 rounded-xl overflow-hidden">
                <div className="bg-slate-50 px-4 py-2 border-b border-slate-100 flex items-center gap-2">
                  ...
                  <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider">
                    Processing Costs / Piece{defaultCur ? ` · ${defaultCur.code}` : ''}
                  </p>
                </div>
                <div className="p-4">
                  {costFields.length === 0 ? (
                    <p className="text-sm text-slate-400 italic text-center py-2">No cost items. Add them in Settings → Cost Breakdown Items.</p>
                  ) : (
                    <div className="grid grid-cols-2 gap-3">
                      {costFields.map(f => (
                        <div key={f.key}>
                          <label className="block text-2xs font-semibold text-slate-400 uppercase tracking-widest mb-1.5">{f.label}</label>
                          <div className="relative">
                            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-xs font-semibold pointer-events-none">{currSym}</span>
              ...
              <div className="border border-slate-200 rounded-xl p-4 space-y-3">
                <div className="flex items-center justify-between">
                  <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Profit Margin</p>
                  {unitCostCalc > 0 && (
                    <span className="text-xs text-slate-400">
                      Unit cost: <span className="font-semibold text-slate-700">{fmtCalc(unitCostCalc)}</span>
                    </span>
                  )}
                </div>
                <div className="relative">
                  <input type="number" min="0" step="any" value={profitMargin}
                ...
                <div className="flex flex-wrap gap-1.5">
                  {PROFIT_PRESETS.map(p => (
                    <button key={p} type="button" onClick={() => setProfitMargin(String(p))}
                      className={`text-xs px-2.5 py-1 rounded-full border font-medium transition-all ${profitMargin === String(p) ? 'bg-indigo-600 border-indigo-600 text-white' : 'border-slate-200 text-slate-500 hover:border-indigo-300 hover:text-indigo-600'}`}>
                      {p}%
                    </button>
                  ))}
                </div>
              ...
              {/* Result Summary */}
              {unitCostCalc > 0 && (
                <div className="bg-gradient-to-br from-indigo-600 to-indigo-700 rounded-xl p-4 text-white space-y-3">
                  <p className="text-2xs font-semibold text-indigo-300 uppercase tracking-widest">Per-Piece Breakdown · {defaultCur?.code || ''}</p>
                  <div className="space-y-1.5">
                    {fabricCostPerPiece > 0 && (
                      <div className="flex justify-between text-sm">
                        <span className="text-indigo-200">Fabric / Material</span>
                        <span className="font-medium">{fmtCalc(fabricCostPerPiece)}</span>
                      </div>
                    )}
                    {costFields.filter(f => parse(costs[f.key]) > 0).map(f => (
                      <div key={f.key} className="flex justify-between text-sm">
                        <span className="text-indigo-200">{f.label}</span>
                        <span className="font-medium">{fmtCalc(parse(costs[f.key]))}</span>
                      </div>
                    ))}
                    <div className="flex justify-between pt-2 border-t border-indigo-500 text-sm">
                      <span className="text-indigo-100 font-semibold">Unit Cost</span>
                      <span className="font-bold">{fmtCalc(unitCostCalc)}</span>
                    </div>
                    {profitPct > 0 && (
                      <div className="flex justify-between text-sm">
                        <span className="text-indigo-200">+ Profit ({profitPct}%)</span>
                        <span className="font-medium text-emerald-300">+{fmtCalc(profitAmt)}</span>
                      </div>
                    )}
                  </div>
                  <div className="bg-white/15 rounded-xl px-4 py-3 flex items-center justify-between">
                    <span className="text-sm font-semibold text-indigo-100">Selling Price</span>
                    <span className="text-2xl font-black text-white">{fmtCalc(sellingPriceCalc)}</span>
                  </div>

                  {/* View in other currencies */}
                  {calcCurrencies.filter(c => c.code !== defaultCur?.code).length > 0 && (
                    <div>
                      <button
                        type="button"
                        onClick={() => setShowOtherCurrencies(v => !v)}
                        className="flex items-center gap-1.5 w-full text-xs text-indigo-200 hover:text-white transition-colors pt-1"
                      >
                        <Globe size={11} />
                        {showOtherCurrencies ? 'Hide other currencies' : 'View in other currencies'}
                        <ChevronDown size={11} className={`ml-auto transition-transform duration-200 ${showOtherCurrencies ? 'rotate-180' : ''}`} />
                      </button>
                      {showOtherCurrencies && (
                        <div className="bg-white/10 rounded-xl p-3 mt-2 space-y-2">
                          {calcCurrencies.filter(c => c.code !== defaultCur?.code).map(c => {
                            const defRate = parseFloat(defaultCur?.rate_to_pkr) || 1;
                            const tgtRate = parseFloat(c.rate_to_pkr) || 1;
                            const convSell = (sellingPriceCalc * defRate) / tgtRate;
                            const convCost = (unitCostCalc    * defRate) / tgtRate;
                            const sym = c.symbol || c.code;
                            const fmtC = v => `${sym}${v.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`;
                            return (
                              <div key={c.code} className="flex justify-between items-center">
                                <div>
                                  <span className="text-xs font-semibold text-indigo-100">{c.code}</span>
                                  {c.name && <span className="text-2xs text-indigo-300 ml-1">{c.name}</span>}
                                </div>
                                <div className="text-right">
                                  <span className="text-sm font-bold text-white">{fmtC(convSell)}</span>
                                  <span className="text-2xs text-indigo-300 ml-1.5">cost {fmtC(convCost)}</span>
                                </div>
                              </div>
                            );
                          })}
                          <p className="text-2xs text-indigo-300 pt-1.5 border-t border-white/10">
                            All these prices are saved automatically when you click Apply
                          </p>
                        </div>
                      )}
                    </div>
              ...
              <div className="flex items-center justify-between pt-1 pb-2">
                <p className="text-xs text-slate-400">
                  {sellingPriceCalc > 0 ? (
                    <>
                      Saves <span className="font-semibold text-slate-700">{fmtCalc(sellingPriceCalc)}</span> in <span className="font-semibold text-slate-600">{defaultCur?.code}</span>
                      {calcCurrencies.length > 1 && <> + <span className="font-semibold text-indigo-600">{calcCurrencies.length - 1}</span> other {calcCurrencies.length - 1 === 1 ? 'currency' : 'currencies'}</>}
                    </>
                  ) : 'Enter costs above to calculate a price'}
                </p>
                <button
                  onClick={handleApplyCalc}
                  disabled={sellingPriceCalc <= 0 || applying}
                  className="flex items-center gap-2 px-5 py-2 text-sm bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 disabled:opacity-50 transition-colors font-medium shadow-sm"
                >
                  <Check size={14} />
                  {applying ? 'Applying…' : 'Apply Price to Product'}
                </button>
              </div>
    ...
    <div className="flex flex-col animate-page" style={{ height: 'calc(100vh - 8.5rem)' }}>
      ...
      {view === 'products' && (
        <div className="flex-1 min-h-0 flex flex-col lg:flex-row rounded-2xl border border-slate-200 shadow-sm overflow-hidden bg-white">
          ...
          <div className="w-full lg:w-80 flex-shrink-0 flex flex-col border-b lg:border-b-0 lg:border-r border-slate-200">
