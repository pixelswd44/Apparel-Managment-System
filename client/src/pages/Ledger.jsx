      ...
      {data && (
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-5">
          <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-4">
            ...
            <p className="text-2xl font-black text-emerald-700">{pkr(summary.totalCredit)}</p>
          ...
          <div className="bg-rose-50 border border-rose-200 rounded-2xl p-4">
            ...
            <p className="text-2xl font-black text-rose-700">{pkr(summary.totalDebit)}</p>
          ...
          <div className={`border rounded-2xl p-4 ${(summary.netBalance||0) >= 0 ? 'bg-indigo-50 border-indigo-200' : 'bg-rose-50 border-rose-200'}`}>
            ...
            <p className={`text-2xl font-black ${(summary.netBalance||0) >= 0 ? 'text-indigo-700' : 'text-rose-700'}`}>
      ...
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        ...
        <div className="relative">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)}
            ...
            className="pl-8 pr-3 py-2 text-sm border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 w-64" />
        </div>
        <span className="text-xs text-slate-400 ml-auto">{rows.length} entries</span>
      </div>
                ...
                {rows.map((e, i) => (
                  <tr key={i} className="hover:bg-slate-50/60 transition-colors">
                    ...
                    <td className="px-4 py-3 text-right">
                      {e.credit > 0
                        ? <span className="font-bold text-emerald-600">{pkr(e.credit)}</span>
                        : <span className="text-slate-300">—</span>}
                    </td>
                    <td className="px-4 py-3 text-right">
                      {e.debit > 0
                        ? <span className="font-bold text-rose-500">{pkr(e.debit)}</span>
                        : <span className="text-slate-300">—</span>}
                    </td>
