      ...
      <div className="w-[520px] bg-white border-l border-slate-200 shadow-2xl flex flex-col">
        {loading ? (
          ...
          <>
            ...
            {/* Tabs */}
            <div className="flex gap-1 bg-slate-100 p-1 rounded-xl mx-6 mt-4 flex-shrink-0 overflow-x-auto scrollbar-hide">
              {[
                { key: 'Overview',  label: 'Overview' },
                { key: 'Payments',  label: 'Payments' },
                { key: 'Projects',  label: 'Projects' },
                { key: 'Pay',       label: 'Pay', icon: <CreditCard size={13} /> },
              ].map(({ key, label, icon }) => (
                <button key={key} onClick={() => { setTab(key); setPayOk(''); setPayErr(''); }}
                  className={`flex-1 py-1.5 text-sm rounded-lg font-medium transition-all duration-150 flex items-center justify-center gap-1.5 whitespace-nowrap flex-shrink-0 ${
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="bg-indigo-50 border border-indigo-100 rounded-xl px-4 py-3">
                        <p className="text-2xs font-semibold text-indigo-500 uppercase tracking-wider mb-1">Total Paid</p>
  ...
  return (
    <div onClick={onClick}
      className={`flex items-center gap-3 px-4 py-3 border-b border-slate-100 hover:bg-slate-50 cursor-pointer group transition-colors last:border-b-0 ${
        bal > 0 ? 'border-l-[3px] border-l-rose-400' : 'border-l-[3px] border-l-transparent'
      }`}>
      ...
      {/* Name + type */}
      <div className="flex-1 min-w-0">
        <p className="text-sm font-semibold text-slate-900 group-hover:text-indigo-700 transition-colors truncate">{vendor.name}</p>
      ...
      {vendor.rating > 0 && (
        <div className="hidden sm:flex items-center gap-0.5 flex-shrink-0">
          {[1,2,3,4,5].map(n => (
            <Star key={n} size={10} className={n <= vendor.rating ? 'text-amber-400 fill-amber-400' : 'text-slate-200'} />
          ))}
        </div>
      )}
      ...
      <div className="hidden md:flex items-center gap-4 text-xs flex-shrink-0">
        {billed > 0 && (
          <>
            <span className="text-slate-500">Billed <span className="font-semibold text-slate-700">{pkr(billed)}</span></span>
            <span className="text-slate-500">Paid <span className="font-semibold text-emerald-600">{pkr(paid)}</span></span>
          </>
        )}
        {bal > 0 && <span className="font-bold text-rose-600">{pkr(bal)} due</span>}
        {bal === 0 && billed > 0 && <span className="text-emerald-600 font-semibold text-xs">✓ Settled</span>}
      </div>
...
/* keep VendorCard defined (used nowhere now but avoids dead-code errors elsewhere) */
