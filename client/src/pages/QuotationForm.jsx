...
// ── Smart price lookup ────────────────────────────────────────────────────────
// Returns the best price for a product given the document currency and rate table.
function getSmartPrice(p, docCurrency, currencies) {
  try {
    ...
    // 2. Convert from default-currency price
...
function ProductNameInput({ value, products, onChange, onPickProduct, onAddNew, currencies, docCurrency }) {
  const [open, setOpen] = useState(false);
  const ref = useRef();
  ...
  const matches   = trimmed
    ...
    : [];
  const exactMatch   = matches.some(p => p.name.toLowerCase() === trimmed.toLowerCase());
  const showDropdown = open && trimmed.length > 0 && (matches.length > 0 || !exactMatch);
      ...
      {showDropdown && (
        <div className="absolute z-50 top-full left-0 right-0 mt-1 bg-white border border-slate-200 rounded-xl shadow-xl overflow-hidden">
          ...
          {!exactMatch && trimmed && (
            <button type="button"
              onMouseDown={e => e.preventDefault()}
              onClick={() => { onAddNew(trimmed); setOpen(false); }}
              className={`w-full text-left px-4 py-2.5 text-sm text-indigo-600 hover:bg-indigo-50 flex items-center gap-2 font-semibold transition-colors ${matches.length > 0 ? 'border-t border-slate-100' : ''}`}>
              <Plus size={13} className="flex-shrink-0" />
              <span>Add "<strong>{trimmed}</strong>" as new product</span>
            </button>
          )}
        </div>
      )}
...
function QuickAddProduct({ initialName = '', onClose, onCreated }) {
  const [form, setForm]     = useState({ name: initialName, article_number: '', selling_price: '', description: '' });
  const [saving, setSaving] = useState(false);
  const [err, setErr]       = useState('');
  ...
  async function handleSave() {
    ...
    setSaving(true); setErr('');
    try {
      const { data } = await api.post('/products', { ...form, status: 'active', unit: 'pcs' });
      onCreated(data);
      onClose();
    } catch (e) {
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 animate-modal">
        <div className="flex items-center justify-between mb-5">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">It will be saved to your products database</p>
          </div>
        ...
        <div className="space-y-3.5">
          ...
          <Field label="Description">
            <textarea rows={2} value={form.description} onChange={e => set('description', e.target.value)}
              ...
              placeholder="Rib Stop Pants + Double Eva Collar + Woven Label…" />
          </Field>
        </div>
        ...
        <div className="flex gap-3 mt-5">
          <button onClick={handleSave} disabled={saving}
            ...
            {saving ? <><Loader2 size={14} className="animate-spin" />Adding…</> : <><Check size={14} />Add Product</>}
          </button>
...
function LineItems({ items, onChange, products, setProducts, sym, currencies, docCurrency }) {
  const [quickAdd,     setQuickAdd]     = useState(false);
  const [quickAddName, setQuickAddName] = useState('');
  const [quickAddIdx,  setQuickAddIdx]  = useState(null);
  ...
  function onProductCreated(p) {
    setProducts(prev => [...prev, p]);
    if (quickAddIdx !== null) pickProduct(quickAddIdx, p);
  }

  function openAddNew(name, idx) {
    ...
    <>
      {quickAdd && (
        <QuickAddProduct
          ...
          onClose={() => { setQuickAdd(false); setQuickAddIdx(null); setQuickAddName(''); }}
          onCreated={onProductCreated}
        />
      )}

      {/* Column headers — desktop only; mobile shows per-field labels instead */}
      {items.length > 0 && (
            ...
            <div className="p-4 pr-11 space-y-3 lg:pr-4 lg:space-y-0 lg:grid lg:grid-cols-[1fr_80px_140px_80px_36px] lg:gap-3 lg:items-start">
              ...
              <div className="space-y-2">
                <ProductNameInput
                  ...
                  onPickProduct={p => pickProduct(idx, p)}
                  onAddNew={name => openAddNew(name, idx)}
...
export default function QuotationForm() {
  ...
  const [baseCurrency, setBaseCurrency] = useState('USD'); // base currency from settings
  ...
  // Tracks whether the client_id effect has already fired once (initial data load).
  // We skip the first fire so the saved currency/shipping is never overwritten by
  // the client's default currency when opening an existing quote for editing.
  ...
  useEffect(() => {
    async function init() {
      try {
        ...
        // Default currency = the one flagged is_default=1 in the currencies list
        ...
        if (isEdit) {
          ...
          setForm({
            ...
            company_id:       q.company_id       || defCo?.id || null,
            is_sampling:      !!q.is_sampling,
            shipping_name:    q.shipping_name    || '',
            shipping_phone:   q.shipping_phone   || '',
            shipping_address: q.shipping_address || '',
            shipping_city:    q.shipping_city    || '',
            shipping_country: q.shipping_country || '',
            bank_details:     q.bank_details     ?? bankDef,
            customer_notes:   q.customer_notes   || '',
            terms_conditions: q.terms_conditions ?? termsDef,
          });
          try {
  ...
  // Skip the very first fire — that's always the initial data load setting client_id
  // from the saved quote, not a user action. Without this guard, editing a quote would
  // let the client's default currency stomp over the quote's saved currency.
  ...
  useEffect(() => {
    ...
    setForm(f => {
      // Only override currency if the client has one set AND it exists in our
      // currencies list; on edit we still respect the already-saved currency
      // but allow re-sync if the user switches client.
  ...
  async function handleSaveDraft() {
    ...
    try {
      if (isEdit) {
        await api.put(`/quotations/${id}`, buildPayload('draft'));
      } else {
        await api.post('/quotations', buildPayload('draft'));
      }
      setDirty(false);
      setSaved(true);
  ...
  if (pageLoading) {
    return (
      <div className="flex flex-col items-center justify-center py-32 gap-3">
        ...
        <p className="text-sm text-slate-400">{isEdit ? 'Loading quotation…' : 'Preparing form…'}</p>
      </div>
    );
  }
    ...
    <div>
      ...
      <div className="-mx-8 -mt-8 px-8 py-4 bg-white border-b border-slate-200 sticky top-0 z-30 flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2.5 flex-shrink-0">
          ...
          <button onClick={handleSaveDraft} disabled={savingDraft || saving}
            className="flex items-center gap-1.5 px-4 py-2 text-sm border border-slate-200 bg-white text-slate-600 rounded-xl hover:bg-slate-50 hover:border-slate-300 disabled:opacity-50 transition-all font-medium">
            {savingDraft
          ...
          <button onClick={handleSubmit} disabled={saving || savingDraft}
            ...
            {saving
              ? <><Loader2 size={13} className="animate-spin" />Saving…</>
              : <><Check size={14} />{isEdit ? 'Update Quotation' : 'Create Quotation'}</>
            }
          </button>
        </div>
      </div>
      ...
      <div className="mt-6 space-y-5">
        ...
        {/* ── 1. Quotation Details ── */}
        <SectionCard id="sec-details" icon={FileText} title="Quotation Details">
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              ...
              <Field label={`Quote Currency${form.currency === baseCurrency ? ' (Default)' : ''}`}>
                <div className="relative">
                  <select value={form.currency} onChange={e => set('currency', e.target.value)} className={`${selectCls} pr-10 font-medium`}>
                  ...
                  {form.currency !== baseCurrency && (
                    <span className="absolute right-8 top-1/2 -translate-y-1/2 text-xs font-bold text-indigo-600 pointer-events-none">
              ...
              <div className="sm:col-span-2">
                <Field label="Quotation Number">
                  <input value={form.number} onChange={e => set('number', e.target.value)}
                    ...
                    placeholder={isEdit ? '' : 'Leave blank to auto-generate (e.g. QT-2026-0001)'} />
                </Field>
              </div>
              <Field label="Status">
        ...
        <SectionCard id="sec-terms" icon={FileCheck} title="Terms & Conditions" iconColor="text-rose-600" iconBg="bg-rose-50">
          <textarea rows={8} value={form.terms_conditions} onChange={e => set('terms_conditions', e.target.value)}
            ...
            placeholder={"Production will take 3-4 weeks after order confirmation.\nDelivery will take 5-7 working days after production.\nThis quotation is valid for 15-20 days from the date of issue.\nPayment Terms: 50% advance required to start production."} />
        </SectionCard>
